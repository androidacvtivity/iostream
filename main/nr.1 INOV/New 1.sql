...
SELECT
  ...
  TR.ORDINE,
  TR.RIND,
  COUNT(DISTINCT CASE WHEN A.RIND IN ('1.1.1') AND NVAL(A.COL1) = 1 AND ( (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%')   OR ((ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL  )
  AND ((TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%') OR (TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%') ))   
 THEN A.CUIIO END) AS COL1,
 COUNT(DISTINCT CASE WHEN A.RIND IN ('1.1.2') AND NVAL(A.COL1) = 1 AND ( (TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%' OR TR.RIND LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%')   OR ((ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL  )
  AND ((TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%') OR (TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%') ))   
...
(
    ...
    SELECT 1 AS NR_TABLE, 'CAEM2' AS TABLE_DENUMIRE, NULL  AS PERS_INIT, NULL  AS PERS_FINAL, 'B+C+D+E+H+J+K+G46+M71+M72+M73' AS RIND, 'Total' AS RIND_DENUMIRE, '1' AS ORDINE, 'B+C+D+E+H+J+K+G46+M71+M72+M73' AS RIND_C  FROM DUAL UNION
