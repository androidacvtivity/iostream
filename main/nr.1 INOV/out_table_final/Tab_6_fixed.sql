--
--/* Tab_6 - structured as requested:
--   - Do NOT change outer structure (columns, unions, mapping)
--   - Only change rows/columns logic to 1.6.x
--   - Ensure every inner subquery that feeds alias A exposes COL1..COL8 to avoid ORA-00904
--   - Final meaning:
--       COL1 (output) = SUM(COL2..COL5)
--       COL2 = 1.6.1 (COL1)
--       COL3 = 1.6.2 (COL1)
--       COL4 = 1.6.3 (COL1)
--       COL5 = 1.6.4 (COL1)
--       COL6, COL7 are kept for structure (0)
--*/
--
...
SELECT 
  ...
  /* A.COL1 nefolosit de mapping final: îl păstrăm (total) */
  (COL_16_1 + COL_16_2 + COL_16_3 + COL_16_4) AS COL1,
  /* A.COL2 (=> FINAL COL1) = TOTAL */
  (COL_16_1 + COL_16_2 + COL_16_3 + COL_16_4) AS COL2,
...
(
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN A.RIND = '1.6.1' AND NVAL(A.COL1)=1
         ...
         THEN A.CUIIO END) AS COL_16_1,

    COUNT(DISTINCT CASE WHEN A.RIND = '1.6.2' AND NVAL(A.COL1)=1
         AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
         AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
         THEN A.CUIIO END) AS COL_16_2,

    COUNT(DISTINCT CASE WHEN A.RIND = '1.6.3' AND NVAL(A.COL1)=1
         AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
         AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
         THEN A.CUIIO END) AS COL_16_3,

    COUNT(DISTINCT CASE WHEN A.RIND = '1.6.4' AND NVAL(A.COL1)=1
         AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
         AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
         THEN A.CUIIO END) AS COL_16_4
  ...
  (
    ...
    WHERE
      ...
      D.RIND IN ('1.6.1','1.6.2','1.6.3','1.6.4') AND
      D.CAEM2 NOT LIKE 'A%'
  ) A
...
(
  SELECT 
    ...
    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.1' AND NVAL(D.COL1)=1
         AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL_16_1,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.2' AND NVAL(D.COL1)=1
         AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL_16_2,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.3' AND NVAL(D.COL1)=1
         AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL_16_3,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.4' AND NVAL(D.COL1)=1
         AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL_16_4
  FROM 
    CIS2.VW_DATA_ALL D 
    INNER JOIN (
      SELECT CODUL, FULL_CODE 
...
SELECT 
  ...
  /* A.COL1 nefolosit de mapping final - păstrăm total clasic; A.COL2 (=> final COL1) tot total */
...
(
  SELECT
    ...
    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.1' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
         AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL1,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.2' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
         AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL2,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.3' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
         AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
         THEN D.CUIIO END) AS COL3,

    COUNT(DISTINCT CASE WHEN D.RIND = '1.6.4' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
         AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
