...
FROM 
  (
    SELECT DISTINCT
      D.CUIIO,
      D.CAEM2,
      CIS2.NVAL(SUM(CASE WHEN D.CAPITOL||'.'||D.RIND IN ('1040.1.3.1','1040.1.3.2')   THEN D.COL1 *BS.CIF_IT ELSE NULL END)) AS COL1_1,
      CIS2.NVAL(SUM(CASE WHEN D.CAPITOL||'.'||D.RIND IN ('1040.1.3.1')             THEN D.COL1*BS.CIF_IT  ELSE NULL END)) AS COL2_1,
      CIS2.NVAL(SUM(CASE WHEN D.CAPITOL||'.'||D.RIND IN ('1040.1.3.2')            THEN D.COL1*BS.CIF_IT ELSE NULL END)) AS COL3_1
    FROM
      CIS2.VW_DATA_ALL D  
        INNER JOIN
        CIS2.X_BAZA_SONDAJ BS ON D.CUIIO=BS.CUIIO AND 2023 = BS.ANUL
    WHERE
