  ...
  (
    ...
    (
      SELECT
        ...
        SUM(CASE WHEN A.RIND = '1.11.1' AND NVAL(A.COL1)> 0
        ...
        SUM(CASE WHEN A.RIND = '1.11.2' AND NVAL(A.COL1)> 0
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
             THEN NVAL(A.COL1) END) AS COL_18_2,

        SUM(CASE WHEN A.RIND = '1.11.3' AND NVAL(A.COL1)> 0
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
             THEN NVAL(A.COL1) END) AS COL_18_3,
             
             COUNT(DISTINCT CASE WHEN A.RIND LIKE '1.11%' AND NVAL(A.COL1)> 0
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
    ...
    (
      SELECT 
        ...
        SUM(CASE WHEN D.RIND = '1.11.1' AND NVAL(D.COL1)>=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL_18_1,

        SUM(CASE WHEN D.RIND = '1.11.2' AND NVAL(D.COL1)>=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL_18_2,

        SUM(CASE WHEN D.RIND = '1.11.3' AND NVAL(D.COL1)>=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL_18_3,
             
           COUNT(DISTINCT CASE WHEN D.RIND LIKE '1.11%' AND NVAL(D.COL1)>=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
    ...
    (
      SELECT
        ...
        SUM(CASE WHEN D.RIND = '1.11.1' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)>=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL1,

        SUM(CASE WHEN D.RIND = '1.11.2' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)>=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL2,

        SUM(CASE WHEN D.RIND = '1.11.3' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)>=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN NVAL(D.COL1) END) AS COL3,
             
           

 COUNT(DISTINCT CASE WHEN D.RIND LIKE  '1.11%' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)>=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
