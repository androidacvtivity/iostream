...
SELECT
 ...
 THEN A.CUIIO END) AS COL9,
  ...
  COUNT(DISTINCT CASE WHEN A.RIND IN ('2.2.3') AND NVAL(A.COL2) = 1  AND ((ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL  )
  AND ((TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%') OR (TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%') )   
