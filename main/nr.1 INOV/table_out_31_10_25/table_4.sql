...
SELECT
 ...
 THEN A.CUIIO END) AS COL4,
   ...
   COUNT(DISTINCT CASE WHEN A.RIND  LIKE '1.4%' AND ((ROUND(BS.PERS_IT) >= TR.PERS_INIT AND ROUND(BS.PERS_IT) <= TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL  )
  AND ((TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,1,1) ||'%') OR (TR.RIND_C LIKE '%'|| SUBSTR(A.CAEM2,2,2) ||'%') )   
...
SELECT 
 ...
 TR.NR_TABLE,
  ...
  COUNT(DISTINCT CASE WHEN D.RIND LIKE   '1.4%'  AND ( (TR.RIND LIKE '%'|| SUBSTR(D.CAEM2,1,1) ||'%') OR (TR.RIND LIKE '%'|| SUBSTR(D.CAEM2,2,2) ||'%') )  THEN D.CUIIO END) AS COL5
...
COUNT(DISTINCT CASE WHEN D.RIND LIKE '1.4%' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1) = 1 
 AND 
((CC.RIND LIKE '%'|| SUBSTR(D.CAEM2,1,1) ||'%') OR (CC.RIND LIKE '%'|| SUBSTR(D.CAEM2,2,2) ||'%') )   
