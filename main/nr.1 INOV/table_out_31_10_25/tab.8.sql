...
(
  ...
  (
    ...
    SELECT 
      ...
      ORDINE,
      RIND,
      /* Keep structure: COL1/COL2 hold TOTAL; COL3..COL6 are the 3 items */
      (COL_17_8) AS COL1,
      (COL_17_5 + COL_17_6 + COL_17_7) AS COL2,
      (COL_17_5 + COL_17_6 + COL_17_7) AS COL3,  -- mapped to final COL1
      COL_17_5 AS COL4,                          -- mapped to final COL2
      COL_17_6 AS COL5,                          -- mapped to final COL3
      COL_17_7 AS COL6,                          -- mapped to final COL4
      0 AS COL7,
      0 AS COL8
    FROM
    (
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN A.RIND = '1.7.5' AND NVAL(A.COL1)=1
        ...
        COUNT(DISTINCT CASE WHEN A.RIND = '1.7.6' AND NVAL(A.COL1)=1
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
             THEN A.CUIIO END) AS COL_17_6,

        COUNT(DISTINCT CASE WHEN A.RIND = '1.7.7' AND NVAL(A.COL1)=1
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
             THEN A.CUIIO END) AS COL_17_7,
             
             COUNT(DISTINCT CASE WHEN A.RIND LIKE '1.7%' AND NVAL(A.COL1)=1
             AND ((ROUND(BS.PERS_IT) BETWEEN TR.PERS_INIT AND TR.PERS_FINAL) OR ROUND(BS.PERS_IT) IS NULL)
             AND (TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,1,1)||'%' OR TR.RIND_C LIKE '%'||SUBSTR(A.CAEM2,2,2)||'%')
             THEN A.CUIIO END) AS COL_17_8
      ...
      (
        ...
        WHERE
          ...
          D.RIND IN ('1.7.5','1.7.6','1.7.7') AND
          D.CAEM2 NOT LIKE 'A%'
      ) A
    ...
    (
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.5' AND NVAL(D.COL1)=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL_17_5,

        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.6' AND NVAL(D.COL1)=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL_17_6,

        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.7' AND NVAL(D.COL1)=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL_17_7,
             
             COUNT(DISTINCT CASE WHEN D.RIND LIKE '1.7%' AND NVAL(D.COL1)=1
             AND (TR.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR TR.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL_17_8
      FROM 
        CIS2.VW_DATA_ALL D 
        INNER JOIN (
          SELECT CODUL, FULL_CODE 
    ...
    SELECT 
      ...
      /* Internal COL1..COL6; mapping outside uses COL3..COL6 */
       SUM(COL4) AS COL1,
      (SUM(COL1) + SUM(COL2) + SUM(COL3)) AS COL2,
      (SUM(COL1) + SUM(COL2) + SUM(COL3)) AS COL3, -- mapped to final COL1
      SUM(COL1) AS COL4,                           -- 1.7.5 -> final COL2
      SUM(COL2) AS COL5,                           -- 1.7.6 -> final COL3
      SUM(COL3) AS COL6,                           -- 1.7.7 -> final COL4
      0 AS COL7,
      0 AS COL8
    FROM
    (
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.5' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL1,

        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.6' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
             THEN D.CUIIO END) AS COL2,

        COUNT(DISTINCT CASE WHEN D.RIND = '1.7.7' AND C.FULL_CODE LIKE '%'||CC.CS_CUATM||'%' AND NVAL(D.COL1)=1
             AND (CC.RIND LIKE '%'||SUBSTR(D.CAEM2,1,1)||'%' OR CC.RIND LIKE '%'||SUBSTR(D.CAEM2,2,2)||'%')
